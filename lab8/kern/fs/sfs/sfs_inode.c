...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
    ...
    *alenp = 0;
    ...
    if (!write) {
        //判断要访问的位置是否在或已经超出文件末尾
        if (offset >= din->size) {
            return 0;
        }
        //如果计算出来的结束位置超过了文件当前大小，就把endpos截短为文件大小
        if (endpos > din->size) {
            endpos = din->size;
        }
    }
    //指向“按缓冲/偏移读写块”的函数指针，用于处理非对齐或部分块的读/写
    int (*sfs_buf_op)(struct sfs_fs *sfs, void *buf, size_t len, uint32_t blkno, off_t offset);
    //指向“按整块传输”的函数，用于处理对齐的整块拷贝。
    ...
    //计算从起始块到结束位置跨过了多少个块边界
    uint32_t nblks = endpos / SFS_BLKSIZE - blkno;  // The size of Rd/Wr blocks

  //LAB8:EXERCISE1 2313247 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
